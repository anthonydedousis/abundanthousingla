...
setwd("~/Documents/AHLA Blogging/171112 - Abundant Mapping LA")
permits <- read.csv("permits103018.csv")
...
permit_summarydf <- permits[c(2,4,8,12)]
head(permit_summarydf)
sum(permit_summarydf$rdus) # should be 83,530

permits_agg <- permit_summarydf %>%
  group_by(zip, X50.units, period) %>%
  summarize(rdu_total = sum(rdus))
head(permits_agg)
nrow(permits_agg)

permits2 <- merge(permits_agg, zipcode, by = 'zip') #merge RDU and ZIP code database w/lat-longs by ZIP
head(permits2)

aggregate(rdu_total~zip, data=permits2, FUN=sum) # check totals
...
ggmap(la_map) + geom_point(data = permits2, mapping = aes(x = longitude, y = latitude, color = rdu_total)) + scale_fill_distiller("RDUs Permitted", palette = "Spectral") #dots are colored by rdu_count
...
permits3 <- zips_lacity %>% inner_join(permits2, by = 'zip') # combine ZCTA info with RDU count and lat-longs

# let's make some maps!
# map of all buildings permitted in Los Angeles, 1/1/13-10/15/18, by ZIP, colored by # RDUs
...
# map of all buildings permitted in Los Angeles, 1/1/18-10/15/18, by ZIP, colored by # RDUs
permits2018 <- subset(permits3, period == "2018") # limit to 2018 only
head(permits2018)
sum(permits2018$rdu_total) # should total 13,354

map5a <- ggmap(la_map) +
  geom_sf(aes(fill = cut(rdu_total,c(0,100,250,500,1000,5000), include.lowest = F, dig.lab=10)), data = permits2018, lwd = 0.05, #omg yes lwd draws the lines, finally
          colour = 'black', inherit.aes = F, size = 0, alpha = 0.5) +
  coord_sf(ndiscr = F) +
  scale_fill_brewer(name = "RDUs Permitted", palette=8) +
  theme_bw()
...
# map of all buildings permitted in Los Angeles with 50+ units, 1/1/13-10/15/18, by ZIP, colored by # RDUs
permits50 <- subset(permits3, X50.units == "Yes") # limit to 50+ only
head(permits50)
sum(permits50$rdu_total) # should total 52,075

ggmap(la_map2) +
  geom_sf(aes(fill = cut(rdu_total,c(0,500,1000,2000,5000,10000), include.lowest = F, dig.lab=10)), data = permits50, lwd = 0.05, #omg yes lwd draws the lines, finally
          colour = 'black', inherit.aes = F, size = 0, alpha = 0.5) +
  coord_sf(ndiscr = F) +
  scale_fill_brewer(name = "RDUs Permitted", palette=8) +
  theme_bw()

# map of all buildings permitted in Los Angeles with 50+ units, 1/1/18-10/15/18, by ZIP, colored by # RDUs
permits50_2018 <- subset(permits2018, X50.units == "Yes") # limit to 50+ only
head(permits50_2018)
sum(permits50_2018$rdu_total) # should total 7,536

map6 <- ggmap(la_map) +
  geom_sf(aes(fill = cut(rdu_total,c(0,200,500,5000), include.lowest = F, dig.lab=10)), data = permits50_2018, lwd = 0.05, #omg yes lwd draws the lines, finally
          colour = 'black', inherit.aes = F, size = 0, alpha = 0.5) +
  coord_sf(ndiscr = F) +
  scale_fill_brewer(name = "RDUs Permitted", palette=8) +
  theme_bw()
...
ggsave(filename="map5a.png", plot=map5a)
ggsave(filename="map5b.png", plot=map5b)
ggsave(filename="map6.png", plot=map6)
